...
creates five worker processes using \texttt{fork()} (line~109) and
establishes two pipes per worker for communication. Each worker executes
the \texttt{worker()} function (\texttt{main.c}:42), where it waits for
...
\texttt{Result} structures (\texttt{main.c}:17--26), which ensures
...
\end{lstlisting}
  ...
  \item[Semantics]
    ...
    constructs a \texttt{SentenceMsg} in \texttt{main.c} (lines~209--212)
  ...
  \item[Invariants]~
    \begin{itemize}
      \item The worker reads exactly \texttt{sizeof(SentenceMsg)} bytes
            using \texttt{read\_full()} (\texttt{main.c}:28--40).
  ...
  \item[Error handling]
    ...
    the \texttt{worker()} function (\texttt{main.c}:42), \texttt{read\_full()}
    ...
    parent after all jobs have been processed (\texttt{main.c}:239--245).
...
\end{lstlisting}
  ...
  \item[Semantics]
    ...
    (\texttt{main.c}:42) using \texttt{predict()} from \texttt{model.c}
  ...
  \item[Error handling]
    The program uses \texttt{read\_full()} (\texttt{main.c}:28--40) to
...
\texttt{fork()} in \texttt{main.c} (lines~108--135) after all pipes are
...
(\texttt{main.c}:42) and blocks on \texttt{read\_full()} waiting for
...
(\texttt{main.c}:214). To avoid deadlock, the parent tracks the number of
active jobs using the \texttt{active\_jobs} variable
(\texttt{main.c}:164). The parent sends tasks freely until all five
...
sending the next task (\texttt{main.c}:187--197).
...
(\texttt{main.c}:189--191). After all tasks have been dispatched and all
results collected (\texttt{main.c}:226--236), the parent sends a
zero-length termination message to each worker
(\texttt{main.c}:239--245) and uses \texttt{waitpid()} to collect all
child processes (\texttt{main.c}:250--253), ensuring no zombie processes
remain.
...
\begin{enumerate}[label=(\roman*)]
  ...
  file.}
  In the \texttt{worker()} function (\texttt{main.c}:42), each worker
  ...
  \texttt{main.c}:58 (worker side) and \texttt{main.c}:214 (parent
  ...
  checked for \texttt{NULL} (e.g., \texttt{main.c}:142, 149, 159). If
  ...
  (lines~168--222). Empty sentences are skipped (line~179), and sentences
  longer than \texttt{MAX\_SENTENCE} are truncated (line~180) before
...
I trained the sentiment analysis model in Python using PyTorch, which is available at \url{github.com/pythomiere/SAM}. I implemented the full C inference pipeline, including the neural network forward pass, the binary weight and vocabulary loaders, the tokenizer and the shared header. I also designed the binary serialization format used by \texttt{weights.bin} and \texttt{vocab.bin}.

\medskip
\textbf{Prabeer Sohal.}

I wrote the \texttt{Makefile} which compiles all source files with \texttt{-Wall -Wextra -g} and links the math library. I also wrote this project report including the project descriptions, communication protocol documentation, and all other written sections. Additionally, I assisted with testing and debugging the program to make sure it ran correctly.

\medskip
\textbf{Rian Mehta.}

I wrote \texttt{main.c}, which contains the multi-process working pool including process creation \texttt{fork()}, pipe setup, round-robin task distribution, backpressure handling, and graceful worker shutdown. I also created the architecture diagram and produced the demo video. I also assisted with integration testing and debugging across the full pipeline.
